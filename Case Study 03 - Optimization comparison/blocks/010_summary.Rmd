...
A fim de solucionar problemas de otimização, heurísticas são utilizadas a fim de encontrar uma solução viável em tempo hábil, dado que algoritmos para encontrar a solução ótima exata levariam anos para obter sucesso. Uma dessas heurísticas são algoritmos baseados em população, que utilizam um conjunto de soluções-candidatas e, com uma séria de operações de variação e seleção nelas, exploram o espaço amostral do problema em busca do ponto ótimo. Neste ensaio, utiliza-se o método de evolução diferencial.

O trabalho em questão explora como duas diferentes configurações ([tabela 1](#tabel1)) do método citado impactam no resultado, observando qual destas é a melhor. Para testar as configurações, utiliza-se uma função de Rosenbrock, com o intervalo de dimensões de interesse de $[2,150]$.
